...
%hidden images are tested. The masks are phase-only.
clear;
N=5; %N-1: number of cascaded phase-only masks (L=N-1); the last mask is conjugate(P)
size1=512;
size2=512;
...
phasemask=exp(1i*2*pi*zeros(size1,size2,N)); %initial values for the cascaded phase-only masks
...
temp1=inputall(:,:,1);
for kk=1:N
    temp1=angular_spectrumnew(psize,lamda,temp1,dist);%linear convolution is used
    temp1=temp1.*phasemask(:,:,kk);
end
finalmag=abs(temp1);
