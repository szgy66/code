...
%hidden images are tested.The masks are complex modulation (both amplitude and phase).
clear;
N=5; %N-1: number of cascaded amplitude-phase masks (L=N-1); the last mask is conjugate(P)
size1=512;
size2=512;
...
dist=0.05;%distance between neighboring amplitude-phase masks
lamda=532e-9;%wavelength
psize=20e-6;%pixel size

commask=ones(size1,size2,N);%initial values for the cascaded amplitude-phase masks
...
%Wavefront-matching algorithm for designing the amplitude-phase masks
for iter=1:30 %number of iterations in the optimization
    ...
    for ii=1:N %1:N
        ...
        if ii<N
            commask(:,:,ii)=summation1./summation2;
        else
            commask(:,:,ii)=exp(1i*angle(summation1));
        end
    end
end

%save the amplitude-phase masks
%save commasknew.mat commask
...
%ssimvalue=ssim(vnorm,reference)
